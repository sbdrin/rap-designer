		...
		<draggable tag="el-collapse"
			...
			<el-collapse-item v-for="(item,index) in list"
				...
				<template v-slot:title>
					...
					<i class="el-icon-delete handle-del"
					   v-show="currentIndex === index"
					   @click.stop.prevent="del(item)"></i>
				</template>
				<slot :data="item"></slot>
			</el-collapse-item>
		</draggable>
		<el-button v-if="!isContainer"
				   type="text"
				   icon="el-icon-plus"
				   @click="add">新增列表项</el-button>
	</div>
</template>
-
<script>
	...
	methods: {
		...
		del(data) {
			if (this.isContainer && this.cutomDelCallback) {
				const del = this.list.find(item => item.id === data.id);
		...
		add() {
			const addData = Object.assign({}, this.list[this.list.length - 1], { id: uuid() });
