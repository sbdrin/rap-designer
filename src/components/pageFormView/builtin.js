...
export const createItem = (h, item, vm) => {
	...
	switch (item.type) {
						...
						options={{
							...
							beforDialogHideCallBack: () => {
								return new Promise((resolve, reject) => {
									const _value = vm.$refs.codeEditorRef.getValue();
		...
		case 'chartStyleSetting':
			...
			const { chartConfig } = vm.handlerData('props', 'get');
			// eslint-disable-next-line no-case-declarations
			const key = vm.$store.state.currentPlugins[0].key;
				...
				>
					...
					<pageDialogView
						...
						on-confirmDialog={(id) => {
							...
							const currentPlugins = vm.$store.state.currentPlugins[0];
