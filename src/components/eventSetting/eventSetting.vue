<template>
	<div class="event-setting">
		...
		<div class="event-setting-select">
			<el-dropdown v-if="componentsEvents.length > 0"
						 trigger="click">
				<el-button size="mini">组件事件</el-button>
				<el-dropdown-menu slot="dropdown"
								  class="event-setting-dropdown">
					<el-dropdown-item v-for="item in componentsEvents"
									  :key="item.eventName"
		...
		<div class="event-setting-select">
			<el-button size="mini"
					   @click="openLinkageEventSetting">组件联动事件</el-button>
		</div>
		<div class="event-setting-list">
			<el-table :data="linkageEventData"
					  border
					  size="mini"
					  @cell-click="tableCellClick">
				<el-table-column prop="eventName">
		...
		<pageDialogView :options="pageDialogOptions"
						ref="pageDialogViewRef"
						@cancelDialog="cancelDialogFn"
						@confirmDialog="confirmDialogFn">
			<el-steps :active="linkageEventSettingActiveIndex"
			...
			<div class="linkage-dialog-title">{{currentLinkageEvent.length ? '联动事件关系链: ' + linkageEventLevel : '联动事件关系链: 请选择组件事件'}}</div>
			...
			<div class="linkage-dialog-tool">
				<el-button size="mini"
						   v-show="linkageEventSettingActiveIndex > 0"
						   @click="previous">上一步</el-button>
			</div>
		</pageDialogView>

		<el-drawer title="事件回调函数在线编辑"
				   :visible.sync="drawer"
				   direction="rtl"
...
const linkageComponentEventTypeDisabled = false;
...
export default {
	...
	methods: {
		...
		createEventConfig() {
			const _linkageEvents = [];
			const _componentsEvents = [];
			const _componentsEventsObj = {};
		...
		selectLinkageComponent(item) {
			...
			this.currentLinkageEvent = [...eventSettingIns.linkageEventLevel];
			// 获取联动组件对应数据配置
			const linkageEventConfig = this.pagePulgin.find(pluginItem => pluginItem.id === item.id).custom.linkageEventConfig;
			this.updateLinkageEventConfigCheck(linkageEventConfig);
			this.next();
		},
		selectLinkageComponentEventType(item) {
			const currentEvent = eventSettingIns.setLinkageEventLevel(item.id, 2);
		...
		updateLinkageEventConfigCheck(linkageEventConfig) {
			const findLinkageEventData = eventSettingIns.findLinkageEvent();
