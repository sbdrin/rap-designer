...
export default {
	...
	methods: {
		...
		addScrollEvent() {
			...
			const y = document.querySelector('.' + this.customClassName + ' .iScrollVerticalScrollbar');
			const x = document.querySelector('.' + this.customClassName + ' .iScrollHorizontalScrollbar');
			const $$yy = document.querySelector('.' + this.customClassName + ' .iScrollVerticalScrollbar .iScrollIndicator');
				...
				this.scrollbarIns = new IScroll(
					...
					Object.assign(
						{},
						{
							...
							disableTouch: false,
							disableMouse: true,
							click: !!isAndroid // 处理iscroll 默认禁用了click事件，但是在ios上面是可以使用的
						},
						this.options
					)
				);
