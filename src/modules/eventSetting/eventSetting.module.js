...
export default class EventSetting {
	...
	createEventListener() {
		const eventData = [];
		const linkageEventData = [];

		this.eventData = Object.keys(this.eventListener).forEach((item) => {
			const eventItem = { eventName: item, eventCallBack: this.eventListener[item] };
	...
	createEventFunction(item, functionName) {
        ...
        return function ${functionName || 'on' + firstUpperCase(item.eventName) + 'EventCallBack'}(currentComponentInstance, ...arg) {
          /**
	...
	delEventListener(eventName) {
		const eventListener = Object.assign({}, this.eventListener);
	...
	setLinkageEventLevel(data, index) {
		...
		const currentLinkageEventKey = this.linkageEventLevel.join('$$');
		const currentEventListener = this.eventListener[currentLinkageEventKey];
	...
	findLinkageEvent() {
		...
		if (this.linkageEventLevel.length >= 3) {
			const linkageEventLevel = [...this.linkageEventLevel];
